...
struct WrongDimensionsException : public std::exception {
  const char* what() const throw() { return "Vectors have wrong dimension"; }
};

template <typename T>
bool check_if_zero(const std::vector<T>& v) {
...
bool operator||(const std::vector<double>& lhs,
  ...
  // Любую пару векторов, один из которых равен нулевому вектору будем считать
  // коллинеарными.
  if (check_if_zero(lhs) || check_if_zero(rhs)) {
    return true;
  }
  ...
  // найдем ненулевые компоненты для нахождения коэффициента
  while (k < n1) {
    if (lhs[k] == 0 && rhs[k] == 0) {
      k++;
    } else if ((lhs[k] == 0 && rhs[k] != 0) || (rhs[k] == 0 && lhs[k] != 0)) {
      return false;
    } else {
      break;
    }
  ...
  for (size_t i = k + 1; i < n1; i++) {
    if ((lhs[i] == 0 && rhs[i] != 0) || (rhs[i] == 0 && lhs[i] != 0)) {
      return false;
    }
    if (!are_equal(lhs[i] / rhs[i], coef)) {
      return false;
    }
  }
